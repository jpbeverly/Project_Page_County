...
loc_data_output <- loc_within_radius(educationVA_data, healthVA_data, radius_1 = 64373.8)
...
va_basemap <- get_map(location=c(lon = -77.5, lat = 38.8), zoom=6, scale = "auto", source = 'stamen', maptype = "roadmap")
```
...
ggplot(CenterPopCounty_map) + 
 geom_sf(data=CenterPopCounty_map ,aes(fill= mypalette1(HEALTH_INVERSE_DIST_15mile), geometry=geometry), inherit.aes = FALSE) +
  ...
  ggtitle("Health Care Facility Access Index (15 Miles)")
    ```
...
ggmap(va_basemap) +
  ...
  theme(axis.line=element_blank(),axis.text.x=element_blank(),
        ...
        panel.background=element_blank(),panel.border=element_blank(),panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),plot.background=element_blank()) +
  ggtitle("Educational Facility Access Index (20 Miles)") +
